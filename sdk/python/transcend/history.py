...
BASE_URL = "https://api.aevos.com"


class AevosChatHistory(BaseModel):
    request: Dict
...
def get_history(
    aevos_api_key: str, limit: int = 10, cursor: str = None, filter: str = "all"
) -> List[AevosChatHistory]:
    if filter not in ["all", "mine"]:
        raise AevosError("Incorrect filter given, valid values are: 'all', 'mine'")

    query_params = {"limit": limit, "filter": filter}
    if cursor:
        query_params["cursor"] = cursor

    return _make_request(aevos_api_key, query_params)


def get_by_hash(aevos_api_key: str, hash: str) -> Optional[AevosChatHistory]:
    try:
        url = urljoin(BASE_URL, f"/v1/verified/chat/completions/{hash}")
        headers = {"accept": "application/json", "Authorization": aevos_api_key}
...
def _make_request(
    aevos_api_key: str, query_params: Optional[Dict]
) -> List[AevosChatHistory]:
    try:
