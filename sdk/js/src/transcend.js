...
export default class Transcend {
     ...
     *
     * @param {string} apiKey - aevos API Key
    ...
    static getHistory(apiKey, limit, cursor, filter) {
        ...
        let url = 'https://api.aevos.com/v1/verified/chat/completions?limit=' + limit
        if (cursor) {
    ...
    static getByHash(apiKey, hash) {
        let url = 'https://api.aevos.com/v1/verified/chat/completions/' + hash
        return fetch(url, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
